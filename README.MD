...
* [ ] Add a REPL. Should be fine to run files with `import(...)` for now.
...
A lisp-look-alike language. Atoms are treated as string values, delimited strings are treated as a single value.
...
>(50 get(x) 0) // is 'x' between 49 and 1
```
